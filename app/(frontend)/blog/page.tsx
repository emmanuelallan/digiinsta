...
export default async function BlogPage({ searchParams }: BlogPageProps) {
  ...
  const [{ posts }, featuredPost, categories] = await Promise.all([
    getBlogPosts({ limit: 12, categorySlug }),
    categorySlug ? Promise.resolve(null) : getFeaturedPost(),
    getBlogCategories(),
  ]);

  // Filter out featured post from regular posts (only when not filtering by category)
          ...
          <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-3xl text-center">
              <Badge className="mb-6 bg-white/20 text-white backdrop-blur-sm hover:bg-white/30">
                ...
                {activeCategory ? activeCategory.title : "Our Blog"}
              </Badge>

              <h1 className="text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl">
                {activeCategory ? activeCategory.title : "Tips, Guides & Insights"}
              </h1>

              <p className="mt-6 text-lg text-white/90 sm:text-xl">
                {activeCategory?.description ??
                  "Discover productivity tips, template guides, and insights to help you make the most of your digital tools and achieve your goals."}
              </p>
